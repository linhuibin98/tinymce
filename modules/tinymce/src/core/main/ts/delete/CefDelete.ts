...
const getAncestorCe = (editor: Editor, node: Node) => Option.from(getContentEditableRoot(editor.getBody(), node));
...
const getContentEditableRoot = (root: Node, node: Node) => {
...
const paddEmptyElement = (editor: Editor) => {
  ...
  const ceRoot = getContentEditableRoot(editor.getBody(), selection.getNode());
